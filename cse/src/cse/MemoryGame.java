...
public class MemoryGame extends JFrame implements ActionListener, Restartable {
    JButton gameButton;
    JLabel triesLabel;
    JPanel gamePanel;

    int triesLeft = 20;
    String imageFolderPath = "src/Level1/";
    ...
    public MemoryGame() {
        setTitle("Internet and Social Media");
        ...
        if (lastClicked == null) {
            ...
            if (lastClicked != clickedButton ) {
                if (lastClickedImagePath.equals(imagePath)) {
                    ...
                    lastClicked = null;
                    GameSession.getInstance().addScore(5); 
                    if (allCardsMatched()) {
                    ...
                    GameSession.getInstance().addScore(-1); 
                    if (triesLeft == 0) {
